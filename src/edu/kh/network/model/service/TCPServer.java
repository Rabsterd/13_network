...
public class TCPServer {
	...
	//	1. 서버의 포트번호 정함 
		...
		try {
			//	2. 서버용 소켓 객체 생성
			serverSocket = new ServerSocket(port); // 서버용 소켕을 생성하여 포트 결합
			
			//	3. 클라이언트 쪽에서 접속 요청이 오길 기다림
			// - 서버용 소켓은 생성되고나면 클라이언트 요청이 오기 전까ㅣ
			...
			// InputStreamReader : 문자 기반 스트림과 바이트 기반 스트림 열결에 사용되는 스트림
			...
			try {
				// 보조 스트림 close 시 연결된 기반 스트림(is, os)도 같이 close 됨
				if(pw != null) pw.close();
				if(br != null) br.close();
